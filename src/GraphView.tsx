...
function baseNodesEdges(cfg: Config) {
  ...
  const rawEdges = cfg.showReturns
    ? MOCK_GRAPH.edges
    : MOCK_GRAPH.edges.filter(
        (e) => e.style !== "dashed" && e.style !== "dotted"
      );
...
export default function GraphView() {
  ...
  useEffect(() => {
    (async () => {
      const { nodes: ln, edges: le } = await layoutWithElk(
        base.nodes,
        base.edges,
        cfg.compact
      );
  ...
  const onSearch = useCallback(() => {
    const c = SAMPLE_CASES.find(
      (x) => x.id.toLowerCase() === search.trim().toLowerCase()
    );
    if (!c) {
      ...
      nds.map((n) =>
        n.id === c.currentNodeId && /2\s*days/i.test(c.remarks)
          ? {
              ...
              data: {
                ...(n.data as NodeData),
                badges: [...((n.data as NodeData).badges ?? []), "sla"],
              },
            }
          : n
      )
  ...
  const filteredNodes = useMemo(() => {
    ...
    return nodes.map((n) => ({
      ...n,
      hidden: laneOf(n.id) !== cfg.laneFilter,
    }));
  ...
  const paintedNodes = filteredNodes.map((n) => ({
    ...
    style: {
      ...(n.style || {}),
      opacity: activeCaseId ? (highlight.nodes.has(n.id) ? 1 : dimmed) : 1,
    },
  }));
  const paintedEdges = filteredEdges.map((e) => ({
    ...
    style: {
      ...(e.style || {}),
      opacity: activeCaseId ? (highlight.edges.has(e.id) ? 1 : dimmed) : 1,
      strokeWidth: highlight.edges.has(e.id) ? 2.4 : 1.5,
    },
  }));
  ...
  const onExportPng = useCallback(async () => {
    ...
    const dataUrl = await htmlToImage.toPng(el, {
      backgroundColor: cfg.theme === "light" ? "#ffffff" : "#0b0c0f",
    });
    const a = document.createElement("a");
    a.href = dataUrl;
    a.download = "loan-workflow.png";
    a.click();
  }, [rf, cfg.theme]);
  ...
  const activeCase = activeCaseId
    ? SAMPLE_CASES.find((c) => c.id === activeCaseId)
    : null;
  const currentNodeData = useMemo(
    () =>
      currentNode
        ? (MOCK_GRAPH.nodes as NodeData[]).find((x) => x.id === currentNode)
        : null,
    [currentNode]
  );
  ...
  return (
    <div
      ...
      aria-label="Loan approval workflow graph"
      ref={flowRef}
    >
      <div className="topbar">
        ...
        <div className="toolbar">
          <div className="segment" title="Edge type">
            <button
              className={clsx({ active: cfg.edgeType === "smoothstep" })}
              onClick={() => setCfg({ ...cfg, edgeType: "smoothstep" })}
            >
              Smooth
            </button>
            <button
              className={clsx({ active: cfg.edgeType === "step" })}
              onClick={() => setCfg({ ...cfg, edgeType: "step" })}
            >
              Step
            </button>
          </div>
          <button
            className="btn"
            onClick={() => setCfg({ ...cfg, compact: !cfg.compact })}
          >
            {cfg.compact ? "Comfy" : "Compact"}
          </button>
          <button
            className="btn"
            onClick={() => setCfg({ ...cfg, showLabels: !cfg.showLabels })}
          >
            {cfg.showLabels ? "Hide labels" : "Show labels"}
          </button>
          <button
            className="btn"
            onClick={() =>
          ...
          <button className="btn" onClick={onRelayout}>
            Re-layout
          </button>
          <button className="btn" onClick={() => rf.fitView({ padding: 0.2 })}>
            Fit
          </button>
          ...
          <button
            className="btn"
            onClick={() => setCfg({ ...cfg, showLegend: !cfg.showLegend })}
          >
            {cfg.showLegend ? "Legend ▲" : "Legend ▼"}
          </button>
        </div>
      </div>
      ...
      <div style={{ position: "relative" }}>
        {cfg.showLanes && (
          <div className="lanes" aria-hidden="true">
            <div
              className={clsx("lane business")}
              data-label="Business"
              style={{ top: 0, height: "50%" }}
            />
            <div
              className={clsx("lane crm")}
              data-label="CRM"
            ...
            <div style={{ marginTop: 8 }}>
              <svg width="120" height="40">
                <line
                  ...
                  strokeWidth="2"
                />
                <text x="6" y="30" fontSize="11" fill="#6b7280">
                  Forward
                </text>
              </svg>
            </div>
        ...
        <aside
          className={clsx("drawer", drawerOpen && "open")}
          role="complementary"
        ...
        >
          ...
          <div className="section">
            ...
            {activeCase && (
              <>
                <div
                  style={{ marginTop: 12, fontSize: 13, color: "var(--text)" }}
                >
                  <strong>Case:</strong> {activeCase.id} —{" "}
                ...
                <div style={{ fontSize: 13, marginTop: 8 }}>
                  ...
                  <div style={{ marginTop: 8, color: "var(--muted)" }}>
                    Remarks: {activeCase.remarks}
                  </div>
                </div>
              </>
            )}
            <div style={{ marginTop: 16, fontSize: 12, color: "var(--muted)" }}>
              Next actions (hint): forward or send-back depending on node;
              viewer is read-only.
            </div>
            <button
              className="btn"
